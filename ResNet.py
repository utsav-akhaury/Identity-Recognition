# We use the source code of ResNet from Keras
# and replace all Conv2D layers by SeperableConv2D layers.
# Some blocks were modified to reduce the no. of parameters
...
def ResNet(stack_fn,
  ...
  x = layers.SeparableConv2D(64, 7, strides=2, use_bias=use_bias, name='conv1_conv')(x)
...
def block1(x, filters, kernel_size=3, stride=1, conv_shortcut=True, name=None):
  ...
  x = layers.SeparableConv2D(filters, 1, strides=stride, name=name + '_1_conv')(x)
  x = layers.BatchNormalization(
  ...
  x = layers.SeparableConv2D(4 * filters, 1, name=name + '_3_conv')(x)
  x = layers.BatchNormalization(
...
def block3(x,
  ...
  x = layers.SeparableConv2D(filters, 1, use_bias=False, name=name + '_1_conv')(x)
  x = layers.BatchNormalization(
      axis=bn_axis, epsilon=1.001e-5, name=name + '_1_bn')(x)
  x = layers.Activation('relu', name=name + '_1_relu')(x)
  ...
  c = filters // groups
  x = layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name=name + '_2_pad')(x)
  x = layers.DepthwiseSeparableConv2D(
      kernel_size,
      strides=stride,
      depth_multiplier=c,
      use_bias=False,
...
def ResNet_small(include_top=True,
             weights='imagenet',
             input_tensor=None,
             input_shape=None,
             pooling=None,
  ...
  return ResNet(stack_fn, False, True, 'resnet_small', include_top, weights,
